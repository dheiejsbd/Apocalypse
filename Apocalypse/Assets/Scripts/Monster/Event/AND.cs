...
{
    [CreateAssetMenu(menuName = "ScriptableObject/Monster/Event/Gate/AND", fileName = "AND", order = int.MaxValue)]
    public class AND : EventBase
    {
        ...
        public override int EventChack()
        {
            for (int i = 0; i < eventBases.Length; i++)
            {
                if (eventBases[i].EventChack() == -1) return -1;
            }
            return (int)TargetID;
        }
